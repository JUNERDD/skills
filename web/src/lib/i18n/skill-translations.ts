...
const skillTranslations: Partial<Record<Locale, Record<string, SkillTranslation>>> = {
  "zh-CN": {
    ...
    "multitask-coordinator": {
      ...
      workflow: [
        "在已有授权内框定根规格，并在派发 writer 前读取适用规则和 dirty state。",
        "构建带依赖、owner、稳定输入、范围、证据和验收标准的任务与决策图。",
        "需要临时记忆时，在共享容器中创建独占运行目录，并遵守明确指定的运行路径与保留策略。",
        "让当前 planner 保留小型或紧耦合工作；仅为独占且可分解的子树任命 subplanner。",
        "将 worker 绑定到已接受的决策和契约版本，再在所有权、隔离和集成容量内派发有用的就绪工作。",
        "增量检查终态证据，按类型路由冲突，并在验收后立即解锁依赖项。",
        "重试失败的 writer 前，确认其已停止写入，检查部分改动，并提供恢复后的基线。",
        "完成必需且与风险匹配的集成检查后收尾；仅在出现新增改动、失败或未解决疑点时继续验证。",
      ],
      outputs: [
      ...
      guardrails: [
        ...
        "worker 可在已接受契约内自行处理常规、可逆的细节；内部验收不扩大授权，也无需为已授权工作再次请求批准。",
        "不要让普通 worker 继续委派；只有明确任命的 subplanner 才能获得后代容量和范围。",
        "不要扫描、广泛注入、并发编辑或不安全删除临时共享记忆文档。",
        "保护健康 worker 的连续执行；仅在失败、冲突、超时、安全风险、新指令或资源限制等规定条件下干预。",
