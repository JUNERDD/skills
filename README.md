...
Current collection version: [`0.2.16`](./VERSION). Release notes are tracked in [`CHANGELOG.md`](./CHANGELOG.md) and published through GitHub Releases.
...
8. Collect one terminal run or bounded observation window. A contextually clear user completion message immediately starts agent-owned shutdown: use available controls to detach producers and finish logging, freeze the collector, then reconcile persistence and classify hypotheses. If a checkpoint, shutdown, or delivery cannot be confirmed, analyze the saved records as incomplete capture without another user completion action. Request a further run only for an identified causal or verification gap.
...
- Agent-owned collection shutdown from an ordinary user completion message, without an extra click or manual Stop/Freeze; reconcile a complete capture when possible, otherwise freeze the persisted evidence as explicitly incomplete and analyze its usable records
- One mutually exclusive Freeze/Resume control backed by a collector-global recording gate: keep `FROZEN` through analysis and repair, and Resume only for a prepared next pass
