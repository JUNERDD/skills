...
export const SKILLS: SkillDetail[] = [
  ...
  },
  {
    ...
    workflow: [
      ...
      "Collect one terminal run or bounded observation window. A contextually clear user completion message starts agent-owned shutdown immediately: detach producers and finish logging when possible, freeze the collector, then reconcile persistence. If a checkpoint or cleanup cannot be confirmed, freeze the persisted records as incomplete without requiring another user action. Summarize and analyze usable evidence while frozen; request another run only for an identified causal or verification gap.",
    ...
    guardrails: [
      ...
      "Do not require an extra click, blur, navigation, DevTools command, or Dashboard Stop/Freeze solely to end collection after a clear completion message. Attach checkpoint instrumentation to the natural boundary and interpret the whole reply in context. The message ends the handoff; runtime evidence independently determines capture completeness.",
      "Do not treat collector-global `FROZEN` as health failure, product completion, or persistence proof. Prefer producer shutdown and completed logging before Freeze; otherwise record an incomplete cutoff and its evidence gaps. Keep recording frozen through analysis and repair, detach any remaining old producers, and Resume only for the prepared next run.",
