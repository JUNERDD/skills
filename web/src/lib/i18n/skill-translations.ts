...
const skillTranslations: Partial<Record<Locale, Record<string, SkillTranslation>>> = {
  "zh-CN": {
    ...
    debug: {
      ...
      lead:
        "一个 coverage-first 调试与修复系统，提供可机器校验的假设与探针计划、丢失与序列缺口可审计的运行时证据、持续演进的调查账本，以及独立的修复后验证。",
      overview:
        "当只读代码不足以证明问题，且运行时 bug 需要从失败契约一路跟进到已验证修复时使用此 skill。它构建有代码依据的因果图，为每个 material hypothesis 同时记录确认与否定证据，校验带 terminal 或 observation-checkpoint 完成模式的 coverage plan，并让 collector 位置同步与 expected-probe 分析复用该计划。每个活动探针都强制采用固定的 all-occurrences/every-execution 契约：每次发生都恰好形成一个独立序列化事件和一条持久化 NDJSON，字节分批只改变网络封装，不改变事件数。浏览器探针通过 `getOrCreateBrowserDebugTransport` 为同一 page realm 与 collector session 获取唯一的 registry-owned transport，并把唯一获取直接赋给顶层 canonical `const`。每个 producer key 只有一个活动 owner：HMR 必须先 release 当前 lease，再获取 inactive control，保留源序列；仍处于 active 时再次获取会让 run 因冲突失效，绝不会静默替换。传输终止会保留完整或不完整 run 的审计记录，后续获取必须使用新 run ID。静态门禁会屏蔽注释、字符串、模板和可分类的 regex literal，对有歧义的 slash/brace 语法 fail closed，并要求所有有效 sink 都绑定到该 canonical transport；直接 fire-and-forget `/ingest`、steady `keepalive: true`、复制但未使用的 transport、shadow/fake sink、重复 producer、静默 catch 或 occurrence 过滤都会使门禁失败。相关 NDJSON 会先按 run、父 flow、operation、request、子 correlation 与传输连续性摘要，再读取必要的原始事件。除非用户明确要求仅诊断，否则 debug、troubleshoot、fix、repair 或 resolve 请求会继续完成证据充分的修复、独立验证、账本收尾与清理。",
      bestFor: [
      ...
      workflow: [
        ...
        "创建并校验 coverage plan，确保边界、假设、探针、固定的 all-occurrences/every-execution 基数契约、结构化的 payload-only 边界、隐私检查和残余歧义一致；结构上拒绝任意层级的未知键，并通过强制语义矛盾审查拒绝试图覆盖 occurrence 策略的说明文本。",
        "启动或连接日志会话；可使用浏览器的本地启动会自动尝试打开并确认 dashboard，明确无界面、CI、容器内或远程会话则显式关闭。",
        "对共享 causal cuts 与 invariants 插桩，通过编译、浏览器 instrumentation、collector、expected-probe、事件基数和传输前缀门禁，并在每次请求用户复现前复制规范化的 `dashboard-status` 状态与 URL 行。",
      ...
      guardrails: [
        ...
        "不要把同一页面多条并发 direct `/ingest` Pending 正常化；这表示浏览器 instrumentation 绕过了必需的共享 transport。",
        "只有页内内存队列时，不要声称跨 reload、navigation、进程丢失、内存耗尽或存储耗尽不存在事件丢失；只能声明已确认的连续前缀，并明确生命周期丢失边界。",
      ...
      entryPoints: [
        ...
        { description: "屏蔽注释与可分类字面量，对 slash/brace 歧义 fail closed，要求唯一顶层 canonical transport，并保守拒绝 direct/aliased ingest、shadow/fake sink、steady keepalive、静默失败、重复 factory 与常见 occurrence gate。", label: "浏览器 instrumentation 校验器" },
        { description: "启动、确认、恢复和停止本地 collector 与仪表盘会话。", label: "会话助手" },
        { description: "由 realm registry 独占，带 conflict-on-active、release-then-reacquire HMR producer、严格字节帧、持久化高水位和可审计 run 终止的页内队列。", label: "浏览器传输" },
        ...
        { description: "全量 occurrence、producer/transport HMR、终止 run 恢复、严格 frame、checkpoint、超时和重试回归测试。", label: "浏览器传输测试" },
        { description: "Direct/aliased ingest、lexical mask、canonical binding、shadow/fake sink、occurrence gate、keepalive、静默 catch 与 registry factory 回归测试。", label: "浏览器 instrumentation 测试" },
      ],
    },
    "grill-me": {
      category: "计划压力测试",
