...
const skillTranslations: Partial<Record<Locale, Record<string, SkillTranslation>>> = {
  "zh-CN": {
    ...
    debug: {
      category: "运行时调试与修复",
      blurb: "用可审计缺口的证据证明、修复并验证运行时 bug。",
      lead:
        "一个 coverage-first 调试与修复系统，提供可机器校验的假设与探针计划、可审计丢失的运行时证据、持续演进的调查账本，以及独立的修复后验证。",
      overview:
        "当只读代码不足以证明问题，且运行时 bug 需要从失败契约一路跟进到已验证修复时使用此 skill。它构建有代码依据的因果图，为每个 material hypothesis 同时记录确认与否定证据，校验带 terminal 或 observation-checkpoint 完成模式的 coverage plan，并让 collector 位置同步与 expected-probe 分析复用该计划。可使用浏览器的本地会话会自动尝试打开并确认内置 dashboard，并在需要时执行有限次数的 fallback。Dashboard 日志流优先显示可选的人类可读 message，缺失时回退到结构化 event 名称或 probe ID，在不改写原始证据的前提下保持紧凑事件可读。必要的实时事件不会被按数量限制或采样：页内传输为事件分配单调 ID，在幂等确认前持续保留，并可在后续事件继续进入时确认一个无缺口前缀 checkpoint。相关 NDJSON 会先按 run、父 flow、operation、request、子 correlation 与传输连续性摘要，再读取必要的原始事件。除非用户明确要求仅诊断，否则 debug、troubleshoot、fix、repair 或 resolve 请求会继续完成证据充分的修复、独立验证、账本收尾与清理。",
      bestFor: [
