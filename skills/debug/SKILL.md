...
- For the bundled browser transport, stamp the collector's current `recordingGeneration`, retry one stable `batchId`, and treat any terminally acknowledged `discardedEvents` as `continuityBroken`, not persisted evidence.
