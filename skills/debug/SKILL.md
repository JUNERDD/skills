...
Derive the dashboard values mechanically: use `frontend_confirmed` and `true` when a URL exists and the frontend callback is recorded; `disabled` and `false` when a URL exists and auto-open is disabled; `frontend_not_confirmed` and `false` when a URL exists otherwise; and `unavailable`, `unavailable`, and `unknown` when no URL exists. Append the normalized error only when non-empty. Normalize embedded newlines in the dashboard status or error to spaces. Before rendering a browser-capable local user handoff, treat `disabled` as an accidental opt-out: run `open-dashboard`, refresh `dashboard-status`, and surface the refreshed line. If that bounded recovery fails, include its exact URL and error without blocking reproduction, even when the refreshed status remains `disabled`. Skip recovery and proceed directly with `disabled` only when the collector host is verified to have no usable local graphical browser.
