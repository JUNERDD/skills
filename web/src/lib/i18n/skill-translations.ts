...
const skillTranslations: Partial<Record<Locale, Record<string, SkillTranslation>>> = {
  "zh-CN": {
    ...
    debug: {
      category: "运行时调试",
      blurb: "用高覆盖探针和调查账本证明运行时根因。",
      lead:
        "一个要求证明的调试系统，最大化每次复现获得的信息量，用于运行时 bug、回归、偶发行为和昂贵失败。",
      overview:
        "当只读代码不够时使用此 skill。它构建宽但去重的因果假设集，把假设映射到高信息探针，用内置本地 collector 收集相关 NDJSON 证据，在阅读原始日志前先做摘要，通过增量 root-cause 账本证明因果链，应用最小已证明修复，用独立运行验证，然后移除临时 instrumentation。当复现昂贵或偶发时，one-shot 模式取消任意假设/探针数量上限。",
      bestFor: [
      ...
      workflow: [
        ...
        "用 `scripts/debug_session.py` 启动或连接日志会话，再添加带观察成本控制的高信息临时探针。",
        "通过覆盖门禁，收集一次干净复现，并在阅读原始体积前摘要 NDJSON 日志。",
        "将假设标记为确认、否定、不确定或未到达，然后追加 root-cause 调查账本。",
        "只应用已证明的修复，用独立运行验证，然后移除 instrumentation 并停止 collector。",
      ],
      outputs: [
      ...
      entryPoints: [
        ...
        { description: "启动和停止本地 collector 会话。", label: "会话助手" },
    ...
    "receiving-code-review": {
      ...
      guardrails: [
        ...
        "在 intake、条目枚举和初步 dispositions 完成前不要改代码。",
