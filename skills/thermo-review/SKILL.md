...
- Prefer recommendations that delete moving parts over recommendations that only move the same complexity elsewhere.

## 350-Line File Limit

- Treat `350` lines as the default review threshold for a maintained source file.
- Flag any reviewed change that pushes a file from `350` lines or fewer to more than `350` lines unless there is a strong structural reason.
- Also flag growth in files that were already over `350` lines when the diff adds meaningful behavior without decomposition.
- Prefer splitting focused modules, extracting pure helpers, moving ownership to the canonical layer, or deleting complexity before adding more code to an oversized file.
...
3. Review candidates high-yield first: whole-file or boundary issues, then abstraction and ownership issues, then local branch/helper simplifications.
...
- `Blocker`: A structural regression that creates serious ongoing delivery risk, prevents a maintainable merge, crosses the 350-line limit without a defensible reason, or preserves major incidental complexity despite a clear simpler design.
...
2. Build the diff inventory and line-count inventory, including the 350-line threshold check.
...
10. Run the report self-check:
    ...
    - Every maintained source file over `350` lines is accounted for or explicitly waived.
